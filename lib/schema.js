...
module.exports = function(schema, callback) {
  ...
  function createTransform(type, argument, key) {
    return function schemaValidationStep(object, callback) {
      preparers[type](argument, object, key, function(err, object) {
  ...
  for (var key in schema) {
    ...
    for (var schemaValidationType in keySchema) {
      if (!preparers[schemaValidationType])
        return callback(new Error("Invalid schema key on `" + key "`: `" +
...
var preparers = {
  ...
  default: function(default, object, key, callback) {
    if (object[key] == null) object[key] = default;
    callback(null, object);
  },
  ...
  enum: function(enum, object, key, callback) {
    if (object[key] == null) return callback(null, object);
    if (!Array.isArray(enum))
      return callback("Unrecognised `enum` on schema: `" + enum +
                      "` expected an array.");
    if (~enum.indexOf(object[key])) return callback(null, object);
    callback("Expected `" + key + "` to be in [" + enum + "]");
  },
