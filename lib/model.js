...
Model.prototype.setUniqueKey = function(key, returnOldOnConflict) {
  ...
  db.constraints.uniqueness.createIfNone(this.type, key, callback);
};
...
Model.prototype.findAll = function(callback) {
  var self = this;
  this.db.index.readAsList(modelIndex, 'type', this.type, function(err, results) {
    if (err) return callback(err);
    async.map(results, self.compute.bind(self), callback);
  });
}
...
Model.prototype.where = function(predicate, any, callback) {
  ...
  this.db.find(predicate, any, this.cypherStart(), function(err, results) {
    if (err) return callback(err);
    async.map(results, self.compute.bind(self), callback);
  });
}
