...
function getStartFromGraph(graph, opts, params) {
  var self = this;
...
function getMatchFromGraph(graph, opts, params) {
  var self = this;
  ...
  var statements = savedUniqueNodes.map(function(node) {
    var key = {};
    key[node.model.uniqueness.key] = node.props[node.model.uniqueness.key];
    params[node.name + 'key'] = key;
    return '(' + node.name + ':' + node.model.type + ' {' + node.name + 'key})';
  });

  return 'MATCH ' + statements.join(',');
}

function getMergeFromGraph(graph, opts, params) {
  var self = this;
  var savedUniqueNodes = graph.allNodes.filter(function(node) {
...
function getSetFromGraph(graph, opts, params) {
  var self = this;
  ...
  return 'SET ' + updatedNodes.map(function(node) {
...
function getStripRedundantComps(graph, opts) {
...
function getReturnFromGraph(graph, opts) {
...
function cypherCommit(node, opts, callback) {
  ...
  var self = this;
  var params = {};
  var start = getStartFromGraph.call(this, graph, opts, params);
  var match = getMatchFromGraph.call(this, graph, opts, params);
  var merge = getMergeFromGraph.call(this, graph, opts, params);
  var create = getCreateFromGraph.call(this, graph, opts, params);
  var rels = getCreateUniqueFromGraph.call(this, graph, opts, params);
  var set = getSetFromGraph.call(this, graph, opts, params);
  var deleteRedundancies = getStripRedundantComps.call(this, graph, opts);
  var finish = getReturnFromGraph(graph, opts)
  ...
  saveComposition: function saveComposition(root, compName, objects, opts, callback) {
    ...
    async.waterfall([
      ...
      function runCommitAndIndex(objects, cb) {
        ...
        commitAndIndex.call(self, shell, opts, cb);
      },
