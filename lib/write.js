...
function commit(db, object, excludeComps, callback) {
  var isNew = object[db.options.id] == null;
  ...
  if (!isNew) {
    ...
    if (!excludeComps) deleteOldCompositionRels.call(self, txn, object);
  }
...
function commitAndIndex(object, excludeComps, callback) {
  var self = this;
  var comps;
  if (excludeComps) {
    var compNames = _.pluck(this.compositions, 'name');
    comps = _.pick(object, compNames)
    object = _.omit(object, compNames);
  }
  commit.call(this, this.db, object, excludeComps, function(err, savedObject) {
    if (err) return callback(err);
    indexAll.call(self, self.db, object, savedObject, function(err) {
      if (err) return callback(err);
      if (excludeComps) _.extend(savedObject, comps);
  ...
  save: function saveModel(object, excludeComps, callback) {
    ...
    async.waterfall([
      ...
      function (object, callback) {
        commitAndIndex.call(self, object, excludeComps, callback);
      },
