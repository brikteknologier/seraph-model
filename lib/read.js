...
module.exports = {
  ...
  read: function(node, depth, callback) {
    ...
    for (var level = 0; level < depth; ++level) {
      if (!comps) comps = _.values(this.compositions);
    ...
    for (var level = 0; level < depth; ++level) {
      returnVars.push('COLLECT( DISTINCT( level' + level +' ) ) as level' + level);
      returnVars.push('COLLECT( ID( STARTNODE( r' + level + ' ) ) ) as start' + level);
      returnVars.push('COLLECT( ID( ENDNODE( r' + level + ' ) ) ) as end' + level);
      returnVars.push('COLLECT( TYPE( r' + level + ' ) ) as type' + level);
    }
    ...
    this.db.query(query.join(' '), { root: id }, function(err, data) {
      ...
      async.forEach(_.range(depth -1, -1, -1), function(level, callback) {
        ...
        var paths = _.zip(data['start' + level], data['end' + level], data['type' + level]);
        
        async.forEach(paths, function(path, callback) {
          var start = _.find(startNodes, function(node) {
            return node[self.db.options.id] == path[0]
          });
          var end = _.find(endNodes, function(node) {
            return node[self.db.options.id] == path[1]
          });

          var comp = compMapping[level][path[2]];
        ...
        if (err) callback(err);
        else self.compute(node, callback);
      });
    });
  },
