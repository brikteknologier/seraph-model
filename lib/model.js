...
function _commit(db, object, callback) {
  ...
  if (!Object.keys(this.compositions).length) {
    var id = db.save(object, callback);
    return isNew ? id : this.db._getId(object);
  }
...
Model.prototype.read = function(idOrObj, callback) {
  ...
  awaitNodeAndComps.then(function(err, result) {
    if (err) {
      ...
      self.db.index.read(modelIndex, self.type, id, function(err2, result2) {
        if (!err2 && result2 === false)
          return callback(err2, result2); // nonexistent
        // node exist or some other error occured - return original error.
        callback(err);
      });
    } else {
