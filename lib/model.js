...
function _beforeCommit(object, callback) {
  ...
  async.waterfall([
    ...
    _transformComps.call(self, object, object, function(comp, object, cb) {
...
function _commit(db, object, callback) {
  ...
  var awaitValueResolution = awaiter.num(1);
  var didResolveIndex = awaitValueResolution();
  var resolveNode = function(fn) {
    awaitValueResolution.then(function() { fn(rootNode); });
  };
  
  index(function(err) {
    if (err) return callback(err);
    _transformComps.call(self, comps, {}, function(opts, object, cb) {
      ...
      var modelDidResolveValues = awaitValueResolution();

      var resolveNode = _commit.call(opts.model, txn, object, cb);

      resolveNode(function(node) {
        txn.relate(rootNode, opts.rel, node);
        modelDidResolveValues();
      });
  ...
  if (!db.isBatch) {
    awaitValueResolution.then(function() { txn.commit() });
  }
...
function _afterCommit(object, callback) {
  ...
  _transformComps.call(self, object, {}, function(opts, object, cb) {
