...
// Going to need some kind of circular protection here.
function _fetchAllCompositedNodes(db, root, callback) {
  var compKeys = Object.keys(this.compositions);
  if (!compKeys.length) return callback(null, {});

  var self = this;
  var compNamesForRels = {};
  var rels = compKeys.map(function(compName) {
    var relName = self.compositions[compName].rel;
    compNamesForRels[relName] = compName;
    return '`' + relName + '`';
  }).join('|');
  ...
  db.query(cypher, { root: this.db._getId(root) }, function(err, results) {
    ...
    _transformComps.call(self, comps, comps, function(opts, obj, i, cb) {
      _fetchAllCompositedNodes.call(opts.model, txn, obj, function(err, comps) {
...
Model.prototype.read = function(idOrObj, callback) {
  ...
  _fetchAllCompositedNodes.call(this, txn, id, awaitNodeAndComps('comps'));
...
Model.prototype.compose = function(model, compositionName, relName) {
  if (!~this.fields.indexOf(compositionName)) this.fields.push(compositionName);
