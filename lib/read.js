...
module.exports = {
  read: function(node, opts, callback) {
    ...
    this.db.query(query, opts.params, function(err, data) {
      if (err) {
        if ((err.neo4jException && err.neo4jException.match(/EntityNotFound/)) 
            || (err.fields && err.fields[0] && err.fields[0].code && err.fields[0].code.match(/EntityNotFound/))
            || (err.code && err.code.match && err.code.match(/EntityNotFound/))) callback(null, false);
        else callback(err);
        return;
      };
