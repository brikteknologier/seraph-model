...
function getStartFromGraph(graph) {
  var self = this;
  var savedNodes = graph.allNodes.filter(function(node) {
    return node[self.db.options.id] =! null;
  });

  if (!savedNodes.length) return '';

  var statements = newNodes.map(function(node) {
    //todo parameterise here 
    return node.name + '=node(' + node[self.db.options.id] + ')';
  });
...
function cypherCommit(db, node, opts, callback) {
  ...
  var start = getStartFromGraph.call(this, graph);
}
...
module.exports = {
  ...
  createNodeGraph: function(node) { return createNodeGraph.call(this, node) }
};
