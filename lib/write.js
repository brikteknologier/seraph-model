...
var compose = {
  ...
  createUnique: function (graph, opts, params) {
    ...
    function getRels(node) {
      ...
      node.children.forEach(function(childNode) {
        var rel = '-[__rel_' + childNode.name + ':' + childNode.comp.rel + ']-'
        if (childNode.comp.direction == 'out') rel += '>';
        else rel = '<' + rel;

        rels.push(node.name + rel + childNode.name);
  ...
  stripRedundantCompositions: function (graph, opts) {
    ...
    var rel = '-[rel:' + compRels + ']-'
    if (compositions[0].direction == 'out') rel += '>';
    else rel = '<' + rel;

    return [
      compose.continuation(graph),
      'OPTIONAL MATCH ' + graph.name + rel + 'target',
