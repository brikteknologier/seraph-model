...
function reassembleFromGraph(graph, data) {
  function _reassembleFromGraph(children, object) {
    children.forEach(function(child) {
      _reassembleFromGraph(child.children, data[child.name]);
      object[child.comp.name] = data[child.name];
    });
    return object;
  }
  return _reassembleFromGraph(graph.children, data[graph.name]);
}
...
function getStripRedundantComps(graph) {
  ...
  return [
    ...
    'MATCH ' + graph.name + '-[rel:' + compRels + ']->target',
...
function cypherCommit(db, node, opts, callback) {
  ...
  this.db.query(query, params, function(err, result) {
    if (err) return callback(err);
    else return callback(null, reassembleFromGraph(graph, result));
  });
}
...
function commit(db, object, opts, callback) {
  ...
  if (!db.isBatch) {console.log(require('util').inspect(txn.operations)); txn.commit();}
...
module.exports = {
  ...
  cypherCommit: function(node) { return cypherCommit.call(this, null, node) }
};
