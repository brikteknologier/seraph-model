...
function Model(seraphDb, type) {
  ...
  this.on('validate', function(object, callback) {
    if (!schemaObj) return callback();
    schema.validate(schemaObj, object, callback);
  });

  var nodeIndex, idIndex, schemaObj;
  ...
  Object.defineProperty(Model.prototype, 'schema', {
    ...
    get: function() { return schemaObj; },
    set: function(newSchema) {
      schemaObj = schema.parse(newSchema);
      if (schemaObj) this.keys = Object.keys(schemaObj);
      return schemaObj;
    }
  });
...
Model.prototype.timestampFactories = {
  epochSeconds: function() { return moment().unix() },
  epochMilliseconds: function() { return moment().valueOf() }
};

Model.prototype.makeTimestamp = Model.prototype.timestampFactories.epochSeconds;
...
Model.prototype.useTimestamps = function(createdField, updatedField) {
  ...
  this.on('prepare', function addCreatedUpdated(obj, callback) {
    if (!obj[self.createdField]) obj[self.createdField] = self.makeTimestamp();
    obj[self.updatedField] = self.makeTimestamp();
    callback(null, obj);
  });
};
...
module.exports.model = function createSeraphModel(db, type) {
  return new Model(db, type);
};

module.exports.parseSchema = require('./schema');
